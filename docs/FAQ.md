...
- **How do agents declare identity?** Call `watercooler_v1_set_agent(base="<Agent>", spec="<role>")` or pass `agent_func="<platform>:<model>:<role>"` on each write, and include `Spec:` in the entry body. See [STRUCTURED_ENTRIES.md#identity-pre-flight](STRUCTURED_ENTRIES.md#identity-pre-flight).
