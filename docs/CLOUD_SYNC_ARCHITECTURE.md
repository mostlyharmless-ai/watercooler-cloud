# Cloud Sync Architecture - Technical Reference

This document describes the technical implementation of git-based cloud sync for watercooler MCP servers.

**For user setup instructions**, see [CLOUD_SYNC_GUIDE.md](./CLOUD_SYNC_GUIDE.md).

**For decision rationale and strategy**, see [CLOUD_SYNC_STRATEGY.md](./CLOUD_SYNC_STRATEGY.md).

---

## Table of Contents

- [MCP Protocol Constraints](#mcp-protocol-constraints)
- [Core Architecture](#core-architecture)
- [GitSyncManager Implementation](#gitsyncmanager-implementation)
- [MCP Integration](#mcp-integration)
- [Conflict Resolution](#conflict-resolution)
- [Operational Hardening](#operational-hardening)
- [Cloudflare Integration Options](#cloudflare-integration-options)
...
This architectural constraint shapes our entire cloud sync design.
...
1. **Git environment propagation**: Set `GIT_SSH_COMMAND` once and pass `env` to all subprocess calls
2. **Retry logic**: Up to 3 attempts on push failures with automatic pull between retries
3. **Rebase with autostash**: Handles uncommitted work gracefully
4. **Dedicated repo recommended**: Simplifies staging and minimizes diffs
...
### Tool Integration: Write Operations
...
### Tool Integration: Read Operations
...
- Use a standalone repo where the repo root is the threads directory to minimize unrelated diffs and simplify staging.
- If co-located, restrict staging to the `.watercooler/` path.

### Idempotent Writes
- Generate an `Entry-ID` per append and include it in commit message footers and/or the appended markdown.
- Retry logic should detect duplicates and skip re-appending.
...
- Keep status/ball in a YAML front matter block.
- Consider a small `meta.json` per topic if header churn becomes frequent.
- Document last-writer-wins vs merge policy explicitly.

### Cloudflare Worker Safeguards
- Use conditional requests (ETag/If-None-Match and SHA preconditions) for read/write to prevent lost updates.
- R2 cache keys should incorporate commit SHA to avoid stale reads.
- Apply exponential backoff on 429/5xx.

### Observability & SLOs
- Emit metrics: pull/push latency, conflict rate, retry counts, bytes transferred.
- Define target P95 read/write latencies and push-failure SLOs.

### Security
- Prefer deploy keys with least privilege.
- Avoid logging secrets/URLs.
- Consider signed commits (`commit.gpgsign=true`).

### Multi-tenant (Future OAuth)
- Per-user repo naming.
...
See [ENVIRONMENT_VARS.md](./ENVIRONMENT_VARS.md) for complete environment variable reference.
...
- **[Cloud Sync Guide](./CLOUD_SYNC_GUIDE.md)** - User-facing setup instructions
