...
The `watercooler_memory` module provides tools for building a memory graph from watercooler threads, enabling summarization and integration with LeanRAG for semantic search.

## Architecture: Two-Tier Model

The memory module supports a two-tier architecture:

1. **Free Tier (Baseline Graph)**: Local processing with summaries
   - Thread/entry parsing and chunking
   - LLM-generated summaries using local models (Ollama)
   - No external API calls required

2. **Paid Tier (LeanRAG Integration)**: Full semantic search
   - Export baseline graph to LeanRAG format
   - LeanRAG extracts entities via LLM (DeepSeek)
   - LeanRAG generates embeddings on entity descriptions
   - Hierarchical clustering and vector search

**Key insight**: Embeddings are NOT generated by this module. LeanRAG handles entity extraction and embedding generation internally - it only needs `{hash_code, text}` from chunks.
...
config = GraphConfig(
    generate_summaries=True,  # Local LLM summaries
)
...
# Export to LeanRAG format (for paid tier)
...
- **ChunkNode**: Text chunks created by splitting entry bodies for LeanRAG processing
- **EntityNode**: Named entities extracted from chunks (handled by LeanRAG)
...
config = GraphConfig(
    # Enable/disable LLM summary generation (using local Ollama)
...
For summary generation (free tier - local models):
- `OLLAMA_API_BASE`: Ollama API endpoint (default: `http://localhost:11434`)
- `OLLAMA_MODEL`: Model name (default: `qwen2.5:0.5b`)

For LeanRAG integration (paid tier):
- `DEEPSEEK_API_KEY`: API key for DeepSeek LLM (entity extraction)
...
LeanRAG only needs `{hash_code, text}` from chunks. It handles:
- Entity extraction via LLM
- Embedding generation on entity descriptions
- Hierarchical clustering and vector search
...
Note: LeanRAG generates embeddings on entity descriptions internally.
...
The module includes caching for summaries to avoid redundant LLM calls:
...
stats = cache_stats()
print(f"Entry summaries: {stats['summaries']['count']} entries")
print(f"Thread summaries: {stats['thread_summaries']['count']} entries")
...
The checkpoint file is saved atomically after each stage (parsing, chunking, summarization).
...
Even without explicit checkpoints, the disk caches for summaries survive pipeline failures:

- **Summary cache**: `~/.watercooler/cache/summaries/`
- **Thread summary cache**: `~/.watercooler/cache/thread_summaries/`

Re-running `generate_summaries()` automatically reuses cached results, so failed builds can be restarted without re-processing already-completed items.
