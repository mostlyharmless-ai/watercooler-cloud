...
- Add graph-specific metadata (summaries, temporal tracking)
- Support bi-temporal model (event_time from source, ingestion_time when processed)
- Enable projection to LeanRAG format (LeanRAG handles entity extraction + embeddings)
"""
...
class ChunkNode:
    ...
    Chunks are created by splitting entry bodies for LeanRAG processing.
    They inherit temporal attributes from their parent entry.

    Note: Embeddings are generated by LeanRAG on entity descriptions,
    not on chunks directly. Chunks only need hash_code + text for LeanRAG.

    Attributes:
        chunk_id: Hash-based identifier (becomes hash_code in LeanRAG)
...
class EntityNode:
    ...
    Note: This is a placeholder schema. Entity extraction is handled
    by LeanRAG, which generates embeddings on entity descriptions.
    We don't generate entities in the baseline graph.
